...
def main(_):
    ...
    if global_step_file.exists() and int(global_step_file.read_text().strip()) > 0:
        ...
        train_dataset = Dataset.create(**train_dataset_data, freeze=False)
        val_dataset = Dataset.create(**val_dataset_data, freeze=False)
        example_batch = train_dataset.sample(1)
        ...
        while global_step < total_steps:
            ...
            if FLAGS.offline_steps <= global_step < further_training_start:
                if collection_state is None:
                    ...
                    floored = np.floor(train_dataset['oracle_reps'])
                    ...
                    collection_state = dict(ob=ob, done=done, stats=stats, data_to_plot=data_to_plot, new_data_to_plot=new_data_to_plot)
                else:
            ...
            if further_training_start <= global_step:
                ...
                if global_step == further_training_start:
                    ...
                    example_batch = train_dataset.sample(1)
                    agent = _create_agent('fql', FLAGS.seed, example_batch, config)
