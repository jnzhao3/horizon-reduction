...
parser.add_argument('--save_dir', type=str, default='checkpoints/data_collection_cube')
...
parser.add_argument('--flow_restore_path', type=str, default='checkpoints/cube_quadruple_horizon_subgoal_proposer')
...
def dynamical_distance(ob, subgoals, goal_oracle_rep):
    # gamma_to_subgoal: DQC value-based distance from ob to each subgoal (oracle_rep goals)
    ...
    # gamma_to_goal and ob_to_goal: L2 oracle_rep distance (subgoal → goal, ob → goal)
    gamma_to_goal = np.linalg.norm(subgoals - goal_oracle_rep, axis=-1)
    ob_to_goal = float(np.linalg.norm(obs_to_oracle_rep(ob) - goal_oracle_rep))

    return gamma_to_subgoal.reshape(-1), gamma_to_goal, ob_to_goal
...
with tqdm(total=args['num_additional_steps']) as pbar:
    ...
    while num_collected_steps < args['num_additional_steps']:
        ...
        for single_action in action_seq:
            ...
            success = float(terminated or np.linalg.norm(next_oracle_rep - goal_oracle) < 0.2)
