...
class GCValue(nn.Module):
    ...
    def __call__(self, observations, goals=None, actions=None):
        ...
        """
        # print("observations shape:", observations.shape if observations is not None else None)
        # print("actions shape:", actions.shape if actions is not None else None)
        # print("goals shape:", goals.shape if goals is not None else None)
        if self.gc_encoder is not None:
            if goals is None:
...
class FQLActorVectorField(nn.Module):
...
# class GoalProposalVectorField(nn.Module):
#     """Actor vector field for flow policies.
...
#     def setup(self) -> None:
#         self.mlp = self.mlp_class(
#             (*self.hidden_dims, self.goal_dim), activate_final=False, layer_norm=self.layer_norm
#         )

#     @nn.compact
#     def __call__(self, observations, goals=None, times=None, is_encoded=False):
#         """Return the current vector.
...
#                 # inputs = jnp.concatenate([observations, goals], axis=-1)
#         if times is None:
#             inputs = jnp.concatenate([inputs, goals], axis=-1)
#         else:
#             inputs = jnp.concatenate([inputs, goals, times], axis=-1)
...
class RND(nn.Module):
    '''
    Neural network for simple RND bonus.
    '''
    ...
    def setup(self) -> None:
        self.mlp = MLP((*self.hidden_dims, self.embed_dim), activate_final=True, layer_norm=False)

    @nn.compact
    def __call__(self, goals):
        return self.mlp(goals)