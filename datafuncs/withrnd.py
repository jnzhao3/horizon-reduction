...
class WithRND:
    ...
    def create(original_dataset, config, train_dataset, env, seed, save_dir, start_ij, wandb, agent, **kwargs):
        ...
        #     wandb.log({f"data_collection/rnd_init_{k}": v}, step=0)
        ...
        for collect_i in tqdm.tqdm(range(start_i, config['collection_steps'] + 1)):
            ...
            if success: # TODO: update if not navigate
                ...
                # for k, v in rnd_stats.items():
                #     wandb.log({f"data_collection/rnd_{k}": v}, step=collect_i)
            ...
            for k, v in rnd_info.items():
                wandb.log({f"data_collection/rnd_{k}": v, 'collect_i': collect_i}, step=collect_i)
            ...
            if done:
                ...
                wandb.log({f"data_collection/rnd_reward_viz": v, 'collect_i': collect_i}, step=collect_i)
            ...
            if collect_i % config['plot_interval'] == 0:
                for k, v in stats.get_statistics().items():
                    # wandb.log({f"data_collection/{k}": v}, step=collect_i)
                    wandb.log({f"data_collection/{k}": v, 'collect_i': collect_i}, step=collect_i)
                ...
                # wandb.log({"data_collection/replay_buffer_viz": wandb.Image(fig_name)}, step=collect_i)
                wandb.log({"data_collection/replay_buffer_viz": wandb.Image(fig_name), 'collect_i': collect_i})
