...
def evaluate(
    ...
    for i in trange(num_eval_episodes + num_video_episodes):
        ...
        if sampler is not None:
            tran = sampler()
            print("setting to transition..")
            state = {"qpos": tran["qpos"][0], "qvel": tran["qvel"][0]}
            if "button_states" in tran:
                state["button_states"] = tran["button_states"][0]
            observation, info = env.reset(options={"set_state": state})     
        else:
